...
class Sigmoid(Layer):
    ...
    def backward(self):
        ...
        # note to self: we have to take the partial from this layer and multiply by
        # the partial from the n+1 layer
        # (although we are going backwards so it was the "previous" layer)
        # (chain rule)
        # for this specific value the way we've set this network up
        # grad_cost here is the gradient from the MSE layer
        for n in self.outbound_layers:
            grad_cost = n.gradients[self]
            sigmoid = self.value
